...
import com.zhiying.zylibrary.utils.coder.MD5Coder;
import com.zhiying.zylibrary.utils.io.FileUtils;
 ...
 * 描述
 */
public class ZYAppCacheUtils {

    public static final String DEFAULT_CACHE_NAME = "appCache";

    private File mCacheFile;
    private static Map<String, ZYAppCacheUtils> mCacheUtilsMap = new HashMap<>();

    private ZYAppCacheUtils(File cacheFile) {
        mCacheFile =  cacheFile;
        FileUtils.mkdirs(cacheFile);
    }

    public static ZYAppCacheUtils getInstance(Context ctx) {
        return getInstance(StorageUtils.getIndividualCacheDirectory(ctx));
    }

    public static ZYAppCacheUtils getInstance(String path, String cacheDirName) {
        return getInstance(new File(path, cacheDirName));
    }

    public static ZYAppCacheUtils getInstance(Context ctx, String cacheDirName) {
        return getInstance(new File(StorageUtils.getIndividualCacheDirectory(ctx), cacheDirName));
    }

    public static ZYAppCacheUtils getInstance(File file) {
        ZYAppCacheUtils appCacheUtils = mCacheUtilsMap.get(file.getAbsolutePath());
        if (appCacheUtils == null) {
            appCacheUtils = new ZYAppCacheUtils(file);
